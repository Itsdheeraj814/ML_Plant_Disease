...
# Class indices mapping (Adjust as per your model) data base
class_indices = {
    0: 'Apple Scab',
    1: 'Apple Black Rot',
...
def is_plant_image(image_path):
    # Basic heuristic: Check if the image has a dominant green color (plants are generally green)
    img = Image.open(image_path)
    img_array = np.array(img)
    green_pixels = np.sum((img_array[:, :, 1] > img_array[:, :, 0]) & (img_array[:, :, 1] > img_array[:, :, 2]))
    total_pixels = img_array.shape[0] * img_array.shape[1]
    green_ratio = green_pixels / total_pixels
    return green_ratio > 0.2  # Threshold for green dominance
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            if not is_plant_image(file_path):
                flash('The uploaded image does not appear to be a plant or leaf.')
                return redirect(request.url)
            ...
            if confidence < 0.5:  # Adjust this threshold as needed
                flash('The model is not confident in its prediction. Please try another image.')
                return redirect(request.url)
